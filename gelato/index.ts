...
logger.initializeSentry( {dsn: process.env.SENTRY_DSN_GELATO_SYNC, 
...
function chainIsOnChainHarvesting( test: IChainHarvester | IChain) : 
...
const run = async () : Promise< void> => {
	const interval = process.env.INTERVAL_SYNC_GELATO ? parseInt( 
																				process.env.INTERVAL_SYNC_GELATO) : 12;
	if ((new Date().getUTCHours() - 2) % interval)	{
		logger.info( `Not yet time to synchronize Gelato tasks. [interval = ${
																														interval} hours]`);
		return;
	}	
  ...
  const pk = process.env.GELATO_ADMIN_PK!;
	...
	await Promise.allSettled( Object.values( <Readonly< IChains>> require( 
																		'../data/chains.js')).map( 
																		async (chain: IChain | IChainHarvester) => {
	  if (!chain.hasOnChainHarvesting || process.argv[ 2] && chain.chainId != 
