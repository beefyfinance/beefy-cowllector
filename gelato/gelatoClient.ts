...
export class GelatoClient {
  ...
  constructor(
    ...
    return <GelatoClient>(<unknown>(async (): Promise<GelatoClient> => {
      if (_gelatoAdmin.provider) {
        ...
        _logger.info(`  ${this._chain.id.toUpperCase()} - gas price = ${price.div(1e9)}`);
  ...
  public async taskInfo(): Promise<Readonly<Record<string, string>>> {
    ...
    _logger.info(`${this._chain.id.toUpperCase()} - Retrieved ${tasks.length} Task IDs.`);
    ...
    const results: PromiseSettledResult<[string, string]>[] = await Promise.allSettled(
      Object.keys(vaults).map(async name => {
        ...
        try {
          ...
          _logger.info(
            `${this._chain.id.toUpperCase()} - task created for ${name}\n  taskId = ${taskId}`
          );
          await this._gelato.renameTask(taskId, name);
          return [name, taskId];
        } catch (error: unknown) {
          _logger.error(
            `Failed to fully form task for ${name} on ${this._chain.id.toUpperCase()}\n${(<any>(
              error
            )).toString()}`
          );
          throw error;
  ...
  public async deleteTasks(tasks: ReadonlySet<Task>): Promise<Record<string, string> | null> {
    //for each task to be deleted...
    //  for (const task of tasks)
    const results: PromiseSettledResult<[string, string]>[] = await Promise.allSettled(
      Array.from(tasks).map(async (task: Task, index) => {
        try {
          //delete the task
          ...
          await txn.wait();
          _logger.info(`${this._chain.id.toUpperCase()} - task deleted: ${task.name}`);
        } catch (error: unknown) {
          _logger.error(
            `Failed to delete task on ${this._chain.id.toUpperCase()}: ${task.name}\n -> ERROR: ${<
              any
            >error}`
          );
          throw error;
  ...
  public async renameTasks(tasks: ReadonlySet<Task>): Promise<Record<string, string> | null> {
    ...
    const results: PromiseSettledResult<[string, string]>[] = await Promise.allSettled(
      Array.from(tasks).map(async (task: Task, index) => {
        try {
          ...
          _logger.info(`${this._chain.id.toUpperCase()} - task renamed: ${task.name}`);
        } catch (error: unknown) {
          _logger.error(
            `Failed to rename task on ${this._chain.id.toUpperCase()}: ${task.name}\n -> ERROR: ${<
              any
            >error}`
          );
          throw error;
