...
class Hits {
  ...
  add(id: string, type: Readonly<HitType>, detail?: any): void {
    if (!id) return;
...
export class TaskSyncer {
  ...
  public async syncVaultHarvesterTasks(): Promise<void> {
        ...
        TaskSyncer._stratsToHarvest.reduce((map, strat) => {
          if (strat.chain !== this._chain.id || map[strat.earnedToken]) {
            if (strat.chain === this._chain.id)
              _logger.warn(
                `${strat.chain.toUpperCase()} -  Duplicate vault-token: ${strat.earnedToken}`
              );
          } else map[strat.earnedToken] = strat;
          return map;
        }, {} as Record<string, IStratToHarvest>),
      vaultsActive: Readonly<VaultRecord> = this._filterToOnChainHarvestedVaults(vaultsOnChain);
    ...
    const [vaultsMissingTask, existingTasks]: [
      Readonly<VaultRecord | null>,
    ...
    if (existingTasks) {
      const [tasksToDelete, tasksToRename]: [ReadonlySet<Task>, ReadonlySet<Task>] = Object.entries(
        existingTasks
      ).reduce(
        (sets, task: Readonly<[string, _Task]>) => {
          if (!task[1].located) sets[0].add({ taskId: task[0], name: task[1].name });
          else if ('string' === typeof task[1].located)
            sets[1].add({ taskId: task[0], name: task[1].located });
          return sets;
        },
        [new Set<Task>(), new Set<Task>()]
      );
      if (tasksToDelete.size) promiseDeleted = this._gelatoClient.deleteTasks(tasksToDelete);
      if (tasksToRename.size) promiseRenamed = this._gelatoClient.renameTasks(tasksToRename);
    } //if (existingTasks)
    ...
    if (tasksCreated) {
      ...
      names.forEach(name =>
        this._hits.add(name, 'created OCH task', `taskId: ${tasksCreated[name]}`)
      );
    ...
    if (tasksDeleted) {
      ...
      names.forEach(name =>
        this._hits.add(name, 'deleted OCH task', `taskId: ${tasksDeleted[name]}`)
      );
    ...
    if (tasksRenamed) {
      ...
      names.forEach(name => {
        ...
        return this._hits.add(name, 'renamed OCH task', {
          ...
          from: !(prior.startsWith('0x') && prior.length == tasksRenamed[name].length)
            ? prior
            : 'default',
        });
      });
    ...
    _logger.info(
      `${this._chain.id.toUpperCase()} - Tasks created: ${report.created}  Tasks deleted: ${
        report.deleted
    ...
    try {
      await postToDiscord({
        ...
        message:
          `+ Tasks created: ${report.created}\n+ Tasks deleted: ${report.deleted}\n+ Tasks renamed: ${report.renamed}` +
  ...
  ): Promise<[VaultRecord | null, Record<string, _Task>]> {
    //note the tasks currently active on Gelato on this chain
    const tasks: Record<string, _Task> = Object.entries(await this._gelatoClient.taskInfo()).reduce(
        (map, task) => {
          map[task[0]] = { located: false, name: task[1] };
          return map;
        },
    ...
    await Promise.all(
      Object.keys(vaults).map(async vaultName => {
        ...
        if (undefined == tasks[taskId]) {
          ...
          _logger.info(`${this._chain.id.toUpperCase()} - Missing Gelato task for ${vaultName}`);
        ...
        } else tasks[taskId].located = vaultName !== tasks[taskId].name ? vaultName : true;
      })
    ...
    if (dirty)
      _logger.info(
        `\n${this._chain.id.toUpperCase()} - Missing task for ${
          Object.keys(vaultsMissingTask).length
        } vaults.\n`
      );
  ...
  private _filterToOnChainHarvestedVaults(
    vaults: Readonly<Record<string, IStratToHarvest>>
  ): VaultRecord {
