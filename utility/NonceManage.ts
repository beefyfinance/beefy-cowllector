...
import type {TransactionResponse, TransactionRequest} from 
...
export class NonceManage extends NonceManager {
  ...
  async sendTransaction( txn: ethers.utils.Deferrable< TransactionRequest>) : 
    ...
    return super.sendTransaction( txn).then( 
																					(txnResponse: TransactionResponse) : 
																					TransactionResponse =>  {
                    _logger.debug( 
															`txn sent to RPC node, decrementing pending to ${
															this._pending - 1}`);
                    --this._pending;
                    return txnResponse;
									});
