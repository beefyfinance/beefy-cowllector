...
const uniqueByAndNonGov = (objectsArray, key) => {
  ...
  return objectsArray.filter( obj => {
    return seen.has( obj[ key]) || obj.id.endsWith( '-gov') || 
													obj.id.endsWith( '-earnings') ? false : seen.add( obj[key]);
  });
}
...
const getVaults = async fileName => {
  const vaultsEndpoint = 
					`https://raw.githubusercontent.com/beefyfinance/beefy-v2/prod/src/config/vault/${
					fileName}`;
