...
const SettledPromiseRejected = (
  result: PromiseSettledResult<unknown>
): result is PromiseRejectedResult => 'rejected' === result.status;
const SettledPromiseFilled = <type>(
  result: PromiseSettledResult<type>
...
class ChainStratManager {
  ...
  SyncVaults(stratsToHarvest: IStratToHrvst[]): boolean {
    ...
    this.vaults.forEach((vault: IVault) => {
      ...
      const index = stratsToHarvest.findIndex(
        (strat: IStratToHrvst) => vault.strategy === strat?.strategy && vault.id === strat.id
      );
      ...
      if (!strat) {
        ...
        return;
      } else this.encountered.add(vault.id);
      ...
      if (
        onChainHarvest
          ...
          : this.chain.hasOnChainHarvesting && !strat?.noOnChainHrvst
      ) {
        if (NOT_FOUND != index) {
          strat.noOnChainHrvst = !onChainHarvest;
          dirty = true;
          this.hits.add(vault.id, 'on-chain-harvest switch');
        } else strat.noOnChainHrvst = !onChainHarvest;
    ...
    const provider = new ETHERS.providers.JsonRpcProvider(this.chain.rpc),
      results = <readonly PromiseSettledResult<unknown>[]>(
        await Promise.allSettled(
          strats.map((strat: IStratToHrvst) => estimateGas(strat, this.chain.chainId, provider))
        )
      );
...
async function main(): Promise<void> {
  ...
  /*Object.values( <Readonly< IChains>> require( '../data/chains.js')).forEach( (chain: IChain) =>  {*/ await Promise.all(
    Object.values(<Readonly<IChains>>require('../data/chains.js')).map(async (chain: IChain) => {
      ...
      if (added || removed)
        console.log(`Strats on ${chain.id.toUpperCase()}: ${added} added, ${removed} removed`);
      else console.log(`No strats added or removed from ${chain.id.toUpperCase()}`);
