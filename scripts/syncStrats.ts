...
import {settledPromiseFilled, nodeJsError, type OptionalMutable} from 
																													'../utility/baseNode';
import type {IVault, IStratExtendedProperties, IStratToHarvest, IChain, 
							IChains} from './interfaces';
...
logger.initializeSentry( {dsn: process.env.SENTRY_DSN_STRAT_SYNC, 
													tracesSampleRate: 1.0/*, debug: true*/});
//logger.setLevel( Logger.levels.DEBUG);


class ChainStratManager  {
	private static _extendedStratProperties : Readonly< Record< string, 
																			Omit< IStratExtendedProperties, 'id'>>>;
	static	{
		try {
			//load up current individual-strategy harvest-configuration data 
			//	(possibly changed since our last run)
			this._extendedStratProperties = (<OptionalMutable<  
						IStratExtendedProperties, 'id'>[]> require( 
						'../data/stratsExtendedProperties.json')).reduce( (map, strat) => {
			...
			throw new Error( `${(nodeJsError( error) && 'MODULE_NOT_FOUND' === 
												error.code ? 'stratsExtendedProperties.json not found' :
    ...
    this.vaults.forEach( (vault: IVault) => {
      ...
      //if this vault was unknown at the time of our last run...
        ...
        //add it to our list of active vaults, and note the addition in our log 
				//	of changes made
        stratsToHarvest[ vault.id] = strat = { id: vault.id,
                                chain: vault.chain, 
      ...
      if (!onChainHarvest)
        this.notOnChainHarvest.push( strat);
			...
			const extendedProperties = ChainStratManager._extendedStratProperties[ 
																																			vault.id];
			if (extendedProperties && (newStrat || Object.entries( 
																		extendedProperties).some( ([key, value]) => 
																		(<any> strat)[ key] !== value)))	{
...
async function main() : Promise< void> {
	const interval = process.env.INTERVAL_SYNC_STRATS ? parseInt( 
																				process.env.INTERVAL_SYNC_STRATS) : 12;
	if ((new Date().getUTCHours() - 1) % interval)	{
		logger.info( `Not yet time to synchronize stategy info. [interval = ${
																														interval} hours]`);
  ...
  try {
	...
	stratsToHarvest = await <Record< string, IStratToHarvest>> (<unknown> getKey( 
																															REDIS_KEY)) || {};
...
//if (false)
      ...
      logger.info( `  Updating gas-limit values on Cowllector-managed ${
		  ...
		  if (await stratManager.addGasLimits( stratManager.notOnChainHarvest))
		    dirty = true;
      logger.info( `    Finished gas-limit updates on ${
...
//debugger;
  ...
  if (dirty)
		setKey( REDIS_KEY, stratsToHarvest);

  //report any significant changes occurred during this sync, or the lack therof
  const count = Object.keys( hits.hits).length;
	try	{
		const message = count < 50 ? `\n${count ? `\`\`\`json\n${ JSON.stringify( 
							Object.values( hits.hits), null, 2)}\n\`\`\`` : 
							`No significant changes.`}` : '50+ significant changes: see link';
		if (process.env.REDISCLOUD_URL)		//avoid Discord spam during testing
			...
			logger.info( message);
		logger.info( `\nLog of ${count
														} significant changes written to logging channel`);
	} catch (error: unknown)	{
