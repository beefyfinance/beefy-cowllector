...
const unwrap = async (signer, provider, options, minBalance = '0.1') => {
  try {
    ...
    if (wNativeBalance.lte( minBalance))
			return false;
    let wNative = new ethers.Contract( CHAIN.wnative, IWrappedNative, signer);
    let tx;
    try {
...
const uploadToFleek = async report => {
  let input = {apiKey: process.env.FLEEK_STORAGE_API_KEY,
  ...
  do {
    try {
      ...
      if (uploaded.hash) {
        console.log( `New harvest report for ${CHAIN.id.toUpperCase()
														} => https://dweb.link/ipfs/${uploaded.hash}`);
        return uploaded;
      }
      tries++;
...
*/
  ...
  strats.forEach( strat =>	{ if (max < strat.gasLimit)
																strat.gasLimit = max;});
...
const shouldHarvest = async (strat, 
															...
															harvesterPK) => {
  const _24_MINS = 1440,
				STRAT_INTERVALS_MARGIN_OF_ERROR = Number( 
											process.env.STRAT_INTERVALS_MARGIN_OF_ERROR) || _24_MINS;
  ...
  try {
    if (strat.lastHarvest) {
      ...
      if (!isNewHarvestPeriod) {
        ...
        strat.notHarvestReason = 
															'last StratHarvest log within operative interval';
        return strat;
      }
    } //if (strat.lastHarvest)
    ...
    if (!strat.suppressCallRwrdCheck)
      try {
        ...
        const contract = new ethers.Contract(strat.strategy || strat.address, 
																															abi, harvesterPK);
        ...
        if (error.message.includes( 'INSUFFICIENT_INPUT_AMOUNT')) {
          ...
          return strat;
				...
				const message = `callReward failure on ${strat.id || strat.name} / ${
																strat.strategy || strat.address} --> ${error}`;
        console.log( message);
    ...
    try {
      ...
      let callStaticPassed = await contract.callStatic.harvest( {gasPrice,
      ...
      for (const key of Object.keys( KNOWN_RPC_ERRORS)) {
        if (error.message.includes( key)) {
          strat.notHarvestReason = 
												`Strat failed callStatic with a tagged condition --> ${
...
const harvest = async (strat, 
        ...
        if (CHAIN.id === 'aurora') {
          try {
            ...
            if (tx.status === 1)
              console.log( `${strat.id || strat.name
															}:\tharvested step 1/3: Charges Fees - with tx: ${
															tx.transactionHash}`);
          } catch (error) {
          ...
          try {
            ...
            return {
              ...
              status: 'failed',
              message: `${strat.id || strat.name
																}: fail in step 2/3: - Swaps to tokens needed`,
              data: error.message,
            };
          }
          try {
            ...
            return {contract: strat.strategy || strat.address,
										status: 'failed',
										message: `${strat.id || strat.name
														}: fail in step 3/3 - Adds liquidity and deposits`,
										data: error.message};
          } //try
          ...
          if (TRICKY_CHAINS.includes( CHAIN.id)) {
            ...
            while (receipt === null) {
              try {
                ...
                if (receipt === null)
									continue;
                console.log( `${strat.id || strat.name}:\tharvested after try ${
  ...
  try {
    ...
    const stratContract = new ethers.Contract( strat.strategy || strat.address,
																												IStrategy, harvesterPK);
    ...
    if (CHAIN && CHAIN.harvestHourInterval) {
      ...
      try {
        ...
        if (GASLESS_CHAINS.includes( CHAIN.id)) {
          ...
          provider.formatter._block = (value, format) => originalBlockFormatter(
												{gasLimit: ethers.BigNumber.from(0), ...value}, format);
        } //if (GASLESS_CHAINS.includes( CHAIN.id))
        ...
        if (CHAIN.gas.priceCap < gasPrice) {
          console.log( `Gas price on ${CHAIN.id.toUpperCase()
													} currently exceeds our cap of ${
													CHAIN.gas.priceCap / 1e9} gwei. Aborting this run.`);
          return false;
        }
        ...
        strats = stratsShouldHarvest.map( strat => {
											...
											if (tvl_ && tvl_ < TVL_MINIMUM_TO_HARVEST && 
																								strat.createdAt < _2weeksAgo) {
												strat.shouldHarvest = false;
												strat.notHarvestReason = `TVL is lower than minimum: ${
																											TVL_MINIMUM_TO_HARVEST}`;
											}
											return strat;
										});
        stratsFiltered = stratsFiltered.concat( strats.filter( strat => 
																												!strat.shouldHarvest));
        stratsShouldHarvest = strats.filter( strat => strat.shouldHarvest)
        ...
        stratsShouldHarvest = strats.filter( strat => strat.shouldHarvest);
				...
				stratsShouldHarvest = await harvestHelpers.multicall( CHAIN, 
																						stratsShouldHarvest, 'lastHarvest');

        strats = await Promise.allSettled( stratsShouldHarvest.map( strat => 
																shouldHarvest( strat, gasPrice, harvesterPK)));
        strats = strats.filter( result => result.status === 'fulfilled').map( 
																												result => result.value);

        stratsFiltered = stratsFiltered.concat( strats.filter( strat => { 
							if (strat.shouldHarvest)
								return false;
        ...
        console.table( [...stratsFiltered, ...stratsShouldHarvest],
									[strats[ 0].id ? 'id' : 'name', strats[ 0].strategy ? 
									'strategy' : 'address', 'shouldHarvest', 'notHarvestReason']);

        console.log( `Strats to harvest: ${stratsShouldHarvest.length} of ${
																															strats.length}`);

        //AT: my read of this is as a note of the maximum that may be spent on 
				//	this round of harvesting on this chain
        let totalGas = stratsShouldHarvest.reduce( (total, s) => total + Number( 
																													s.gasLimit), 0) / 1e9;
        console.log( `Total gas to use ${(totalGas * gasPrice) / 1e9
														} gwei. Current balance ${balance.div( 1e9)} gwei`);
        ...
        for await (const strat of stratsShouldHarvest) {
          try {
            await unwrap( harvesterPK, provider, {gasPrice}, 
																											CHAIN.wnativeMinToUnwrap);
          } catch (error) {
            Sentry.captureException( error);
          }
          try {
            let options = {gasPrice, gasLimit: ethers.BigNumber.from( 
																															strat.gasLimit)};
            let harvested = await harvest( strat, harvesterPK, provider, 
																																			options);
          ...
          } catch (error) { //AT: flow analysis suggests no errors can be thrown by harvest()
            Sentry.captureException( error);
            console.log( error.message);
          }
        } //for await (const strat of stratsShouldHarvest)
        ...
        strats = strats.map( strat => {
											let harvested = harvestAttempts.find( h => h.contract ===
																						(strat.strategy || strat.address));
											if (harvested)
												strat.harvest = harvested;
											return strat;
										});

        if (strats.length) {
          let success = strats.filter( strat => strat.harvest?.status === 
																																		'success');
          let gasUsed = harvestAttempts.reduce( (total, h) => {
																if (h.data && h.data.gasUsed) total = total.add( 
																				ethers.BigNumber.from( h.data.gasUsed));
																	return total;
          ...
          let report = {strats,
												...
												failed: harvestAttempts.length - success.length + 
																															callStaticFails};
          if (gasUsed.gt( 0)) {
            report.averageGasUsed = gasUsed.div( ethers.BigNumber.from( 
																															success.length));
          }
          ...
          report.profit = currentBalance.add( currentWNativeBalance).sub( 
																									balance.add( wNativeBalance));

          try {
						const message =`- Total strats: ${strats.length
											}\n- Found to harvest: ${report.harvesteds
											}\n  + Successes: ${report.success}\n  + ${
											report.failed ? '**' : ''}Failures: ${report.failed + 
											(report.failed ? '**' : '')}\n- Total gas used: ${
											ethers.utils.formatUnits( report.gasUsed, 'gwei')
						...
						if (process.env.REDIS_CLOUD_URL)	//avoid Discord spam when testing
							try {
								...
								await discordPoster.sendMessage( {type: 'info',
																	title: `New harvest report for ${
																					CHAIN.id.toUpperCase()}`, 
																	message: `${message}\nIPFS link: ${uploaded ? 
																								`https://dweb.link/ipfs/${
																								uploaded.hash}` : 'failed'}`});
							} catch (error) {
								Sentry.captureException( error);
								console.log( `Error trying to send message to broadcast: ${
																															error.message}`);
